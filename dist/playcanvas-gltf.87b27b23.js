...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"js/lib/playcanvas-gltf.js":[function(require,module,exports) {
Object.assign(window, function () {
  ...
  function translateAnimation(data, resources) {
    ...
    data.channels.forEach(function (channel) {
      ...
      var curve, keyType;
      var i, j; // Animation for the same root, organized in one AnimationComponent
      ...
      if (path === 'weights') {
        ...
        for (i = 0; i < numCurves; i++) {
          ...
          if (sampler.interpolation === "CUBIC") curve.type = AnimationCurveType.CUBIC;else if (sampler.interpolation === "STEP") curve.type = AnimationCurveType.STEP;
        ...
        if (sampler.interpolation === "CUBIC") curve.type = AnimationCurveType.CUBIC;else if (sampler.interpolation === "STEP") curve.type = AnimationCurveType.STEP;else if (sampler.interpolation === "CUBICSPLINE") curve.type = AnimationCurveType.CUBICSPLINE_GLTF; // glTF animation keys can be assumed to be serialized in time
        ...
        if (sampler.interpolation === "CUBICSPLINE") {
          ...
          for (i = 0; i < times.length; i++) {
            time = times[i];
            if (path === 'translation' || path === 'scale') value = new pc.Vec3(values[3 * i + 0], values[3 * i + 1], values[3 * i + 2]);else if (path === 'rotation') value = new pc.Quat(values[4 * i + 0], values[4 * i + 1], values[4 * i + 2], values[4 * i + 3]);else // AnimationKeyableType.NUM
              value = values[i];
            keyable = new AnimationKeyable(keyType, time, value);
            keyables.push(keyable);
          }
      ...
      gltf.textures.forEach(function (texture, idx) {
        if (texture.hasOwnProperty('source')) {
          if (texture.source === imageIndex) {
            ...
            if ((!isPowerOf2(image.width) || !isPowerOf2(image.width)) && (t.addressU === pc.ADDRESS_REPEAT || t.addressU === pc.ADDRESS_MIRRORED_REPEAT || t.addressV === pc.ADDRESS_REPEAT || t.addressV === pc.ADDRESS_MIRRORED_REPEAT || t.minFilter === pc.FILTER_LINEAR_MIPMAP_LINEAR || t.minFilter === pc.FILTER_NEAREST_MIPMAP_LINEAR || t.minFilter === pc.FILTER_LINEAR_MIPMAP_NEAREST || t.minFilter === pc.FILTER_NEAREST_MIPMAP_NEAREST)) {
  ...
  function translateMaterial(data, resources) {
    var material = new pc.StandardMaterial(); // glTF dooesn't define how to occlude specular
    ...
    if (data.hasOwnProperty('extensions') && data.extensions.hasOwnProperty('KHR_materials_pbrSpecularGlossiness')) {
      ...
      if (specData.hasOwnProperty('diffuseFactor')) {
        color = specData.diffuseFactor; // Convert from linear space to sRGB space
      ...
      if (specData.hasOwnProperty('specularFactor')) {
        color = specData.specularFactor; // Convert from linear space to sRGB space
      ...
      if (pbrData.hasOwnProperty('baseColorFactor')) {
        color = pbrData.baseColorFactor; // Convert from linear space to sRGB space
    ...
    if (data.hasOwnProperty('emissiveFactor')) {
      color = data.emissiveFactor; // Convert from linear space to sRGB space
  ...
  function translateMesh(data, resources) {
    ...
    data.primitives.forEach(function (primitive) {
      ...
      var weights = null;
      var indices = null;
      var i; // Start by looking for compressed vertex data for this primitive
      ...
      if (primitive.hasOwnProperty('extensions')) {
        ...
        if (extensions.hasOwnProperty('KHR_draco_mesh_compression')) {
          ...
          if (extDraco.hasOwnProperty('attributes')) {
            var extractAttribute = function extractAttribute(uniqueId) {
            ...
            if (dracoAttribs.hasOwnProperty('POSITION')) positions = extractAttribute(dracoAttribs.POSITION);
            if (dracoAttribs.hasOwnProperty('NORMAL')) normals = extractAttribute(dracoAttribs.NORMAL);
            if (dracoAttribs.hasOwnProperty('TANGENT')) tangents = extractAttribute(dracoAttribs.TANGENT);
            if (dracoAttribs.hasOwnProperty('TEXCOORD_0')) texCoord0 = extractAttribute(dracoAttribs.TEXCOORD_0);
            if (dracoAttribs.hasOwnProperty('TEXCOORD_1')) texCoord1 = extractAttribute(dracoAttribs.TEXCOORD_1);
            if (dracoAttribs.hasOwnProperty('COLOR_0')) colors = extractAttribute(dracoAttribs.COLOR_0);
            if (dracoAttribs.hasOwnProperty('JOINTS_0')) joints = extractAttribute(dracoAttribs.JOINTS_0);
            if (dracoAttribs.hasOwnProperty('WEIGHTS_0')) weights = extractAttribute(dracoAttribs.WEIGHTS_0);
          }
      ...
      var getAttribute = function getAttribute(semantic) {
      ...
      gltf.buffers.forEach(function (buffer, idx) {
        if (buffer.hasOwnProperty('uri')) {
          if (isDataURI(buffer.uri)) {
            ...
            var byteString = atob(buffer.uri.split(',')[1]); // write the bytes of the string to an ArrayBuffer

            resources.buffers[idx] = new ArrayBuffer(byteString.length); // create a view into the buffer

            var uint8Array = new Uint8Array(resources.buffers[idx]); // set the bytes of the buffer to the correct values
    ...
    gltf.nodes.forEach(function (node, idx) {
      if (node.hasOwnProperty('children')) {
        node.children.forEach(function (childIdx) {
          var child = resources.nodes[childIdx]; // If this triggers, a node in the glTF has more than one parent which is wrong
      ...
      if (node.hasOwnProperty('mesh')) {
        ...
        meshGroup.forEach(function (mesh) {
          ...
          if (mesh.morph) {
            ...
            meshInstance.morphInstance = morphInstance; // HACK: need to force calculation of the morph's AABB due to a bug
  ...
  function loadGlb(glb, device, done, options) {
    ...
    var length = dataView.getUint32(8, true); // Read JSON chunk
    ...
    var gltf = JSON.parse(decodeBinaryUtf8(jsonData)); // Read the binary buffers
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","js/lib/playcanvas-gltf.js"], null)
//# sourceMappingURL=/playcanvas-gltf.87b27b23.js.map